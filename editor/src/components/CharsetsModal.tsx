...
// The custom charset animations (CH3) have their OWN window
// (CharsetAnimsModal) — opened from here.
...
interface Props {
  ...
  // CH3 — the workshop lives in its OWN window (CharsetAnimsModal);
  // this one only shows the count and opens it.
  animCount: number;
  onOpenAnims: () => void;
  onClose: () => void;
}
      ...
      >
        ...
        <div className="row" style={{ alignItems: "stretch", gap: 12, minHeight: 0 }}>
          ...
          <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: 8, minWidth: 0 }}>
            ...
            <div className="row" style={{ borderTop: "1px solid var(--border, #333)", paddingTop: 8, marginTop: 4, alignItems: "center", gap: 8 }}>
              <b>Animations custom</b>
              <span className="hint">
                {props.animCount
                  ? `${props.animCount} animation(s) au projet`
                  : "aucune animation au projet"}
              </span>
              <button onClick={props.onOpenAnims}>Éditer les animations…</button>
            </div>
          </div>
        </div>
      </div>
