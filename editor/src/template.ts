...
// The hero's battler: the starter charset's three profile frames laid
// into 32x32 vignette cells (the H4 recipe, « Créer une vignette depuis
// un charset ») — the battle screen loops it as the party's idle.
const HEROS_BATTLER_PNG = "";
...
export async function scaffoldProject(root: string): Promise<void> {
  ...
  await mkdir(`${root}/assets/vignettes`, { recursive: true });
  ...
  await writeBinaryFile(
    `${root}/assets/vignettes/heros_battler.png`,
    bytes(HEROS_BATTLER_PNG),
  );
  ...
  const data: ProjectData = {
    ...
    project: {
      ...
      vignettes: ["assets/vignettes/heros_battler.png"],
      /* the NAME list is what saveProject walks to write
         screens/<n>.json — without it the starter battle screen
         never reached the disk and datagen refused every new
         project on its Duel event */
      screens: ["combat_slimes"],
...
// place the monsters visually, its VIGNETTE stands the hero's battler
// (an idle loop cut from the charset — H2b), its script names the
// monsters' database entries for the library, then hands the fight to
// combat_tour. No backdrop yet — a new project fights on black until
// it owns a picture.
