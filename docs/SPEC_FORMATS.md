...
| 0x3A | LISTSEL | widget u8, var u8, flags u8 | **B6** — menu à CURSEUR sur un widget « list » du layout UI (primitive type 7 : items dans ui_ov_label séparés par \n, 1 colonne réservée au curseur '>'). BLOQUANT : affiche le widget (curseur en haut), haut/bas naviguent avec BOUCLAGE, A valide (`vars16[var]` = index 0..n-1), B annule (`vars16[var]` = 255) si flags bit 0. flags bit 1 : le widget RESTE affiché à la fermeture (multi-panneaux — il perd juste son curseur) ; bit 2 : Gauche/Droite sortent aussi (`vars16[var]` = 254 gauche, 253 droite — le script enchaîne sur la liste voisine, le curseur semble sauter de panneau en panneau). Sans bit 1, le widget est recaché à la fermeture. Widget sans primitive liste : commande ignorée (var intacte). Une seule liste active à la fois ; contexte principal uniquement (comme CHOICE). |
